...
결과는 고객사에게 보낼 답변을 먼저 표시한 뒤 핵심 판단, 개발자 검수 요청, 확인 근거와 검수 정보 순으로 작성합니다. 성공한 Git 명령, SSH 우회, 임시 worktree, 메모리 파일이나 이력 저장 같은 내부 작업 로그는 표시하지 않으며, 실패가 답변 신뢰도에 영향을 줄 때만 짧게 알립니다.
