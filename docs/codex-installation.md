...
개발자 확인이 필요한 문의에는 검증된 현재 동작과 근거 있는 권장 방안을 구분해 작성하고, 구현 가능성과 더 적합한 대안을 검수해 달라고 요청합니다. 근거가 부족하면 `권장 방안: 미정`으로 표시합니다. `work-research`는 공수·비용·인원·일정을 산정하지 않으며 같은 Slack 링크를 `$dev-plan`으로 넘깁니다.
