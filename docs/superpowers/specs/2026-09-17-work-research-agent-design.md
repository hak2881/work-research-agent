...
4. Discover the complete repository set for each resolved project from explicit Slack links, project mappings, existing local remotes, and accessible Git hosting metadata. Never scope repositories by the searched person's ownership or authorship.
...
The repository is a Codex plugin with optional Hermes compatibility. `skills/work-history` and `skills/work-research` define the two explicit skill workflows.
...
- The public repository installs as a Codex plugin.
