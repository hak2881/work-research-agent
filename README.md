...
- `$work-research [1|2] <Slack 링크>`: 관련 이력과 현재 근거를 다시 검사합니다. 고객사 답변을 먼저 보여주고 핵심 판단, 개발자 권장안, 결정에 필요한 검수 항목과 근거만 간결하게 정리합니다. 코드 판단 전 원격 기본 브랜치를 최신화하며, 공수와 일정은 산정하지 않고 `$dev-plan`으로 연결합니다. `1`은 재발 방지 관점과 현재 요청 관점을 함께 제시하고, `2`는 고객사가 요청한 내용에 집중합니다.
