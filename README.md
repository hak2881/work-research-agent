...
- `$work-research [1|2] <Slack 링크>`: 관련 이력과 현재 근거를 다시 검사합니다. 코드 판단 전 원격 기본 브랜치를 최신화하고 정확한 SHA를 검수하며, 필요한 Slack 첨부파일이나 연결 문서는 내려받아 확인할 수 있습니다. `1`은 재발 방지 관점과 현재 요청 관점을 함께 제시하고, `2`는 고객사가 요청한 내용에 집중합니다. 번호를 생략하면 `2`로 동작합니다.
...
- `$dev-plan <PRD·WBS·Slack 링크·문서>`: PM에게 전달할 답변을 먼저 작성하고 요구사항, 현재 코드, 과거 결정을 교차 검수합니다. 간단한 검토는 세션에 답변하며, 복잡한 검토는 상세 HTML 보고서를 함께 만듭니다. 개발 가능성, 공수 범위, 의존성, 완료 조건과 작업 목록도 DB에 기록합니다.
