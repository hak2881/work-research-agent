...
- `$work-research [1|2] <Slack 링크>`: 관련 이력과 현재 근거를 다시 검사합니다. 코드 판단 전 원격 기본 브랜치를 최신화하고 정확한 SHA를 검수합니다. 개발자 확인이 필요하면 근거 있는 권장 방안과 함께 구현 가능성·더 나은 대안 검수를 요청하며, 공수와 일정은 산정하지 않고 `$dev-plan`으로 연결합니다. `1`은 재발 방지 관점과 현재 요청 관점을 함께 제시하고, `2`는 고객사가 요청한 내용에 집중합니다.
